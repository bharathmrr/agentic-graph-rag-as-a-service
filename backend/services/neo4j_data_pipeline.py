...
class Neo4jDataPipeline:
    ...
    async def process_document_to_neo4j(
        ...
        try:
            ...
            if "ontology" in document_data:
                ontology_result = await self._store_ontology_data(
                    document_data["ontology"], document_data.get("document_id")
                )
            ...
            return {
                ...
                "ontology_stored": (
                    ontology_result.get("success", False) if ontology_result else False
                ),
                "entities_stored": (
                    entities_result.get("success", False) if entities_result else False
                ),
                "relationships_stored": (
                    relationships_result.get("success", False)
    ...
    async def _store_ontology_data(
        self, ontology_data: Dict[str, Any], document_id: str
    ) -> Dict[str, Any]:
    ...
    async def _store_entities(
        self, entities_data: Dict[str, Any], document_id: str
    ) -> Dict[str, Any]:
                ...
                for entity_type, entity_info in entities_data.items():
                    ...
                    if isinstance(entity_info, dict):
                        ...
                        if not entity_items and "count" in entity_info:
                            ...
                            entity_items = [
                                f"{entity_type}_{i+1}"
                                for i in range(entity_info["count"])
                            ]
    ...
    async def _store_relationships(
        self, relationships_data: List[Dict[str, Any]], document_id: str
    ) -> Dict[str, Any]:
    ...
    async def _create_document_connections(self, document_id: str) -> Dict[str, Any]:
        ...
        try:
            with self.driver.session() as session:
                ...
                result = session.run(
                    query,
                    {"doc_id": document_id, "created_at": datetime.now().isoformat()},
                )
    ...
    async def verify_data_transfer(self, document_id: str) -> Dict[str, Any]:
        ...
        try:
            with self.driver.session() as session:
                ...
                entities_query = (
                    "MATCH (e:Entity {document_id: $doc_id}) RETURN count(e) as count"
                )
