...
class GroqRAGService:
    ...
    async def create_index_from_documents(self, documents: List[str]) -> bool:
        ...
        try:
            ...
            self.index = VectorStoreIndex.from_documents(
                doc_objects, service_context=self.service_context
            )
            ...
            print(
                f"✅ Created vector index from {len(documents)} documents in {processing_time:.2f}s"
            )
    ...
    async def query_with_rag(
        self, query: str, include_entities: bool = True
    ) -> RAGResponse:
        ...
        try:
                    ...
                    source_nodes.append(
                        {
                            ...
                            "score": (
                                float(node.score) if hasattr(node, "score") else 0.0
                            ),
            ...
            confidence_score = 0.0
            if source_nodes:
                confidence_score = sum(node["score"] for node in source_nodes) / len(
                    source_nodes
                )
    ...
    async def stream_rag_response(
        ...
        try:
            ...
            yield {"type": "complete", "response": response.__dict__}

        except Exception as e:
            yield {"type": "error", "message": f"Error: {str(e)}"}

    async def find_similar_entities(
        self, entity_text: str, top_k: int = 5
    ) -> List[Dict[str, Any]]:
...
async def test_groq_integration():
    ...
    try:
        ...
        # Test RAG query
        rag_response = await service.query_with_rag(
            "What is an LPU and how is it used in AI?"
        )
