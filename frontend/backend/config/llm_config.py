...
class LLMConfig:
    ...
    def switch_llm(self, llm_name: str):
        ...
        if llm_name.lower() in ["grok", "gemini"]:
            ...
            pipeline_logger.log_system_check(
                "LLM Switch", True, f"Switched to {llm_name}"
            )
            return True
        return False
    ...
    def test_llm_connection(self) -> Dict:
        ...
        try:
            ...
            if config["api_key"]:
                pipeline_logger.log_system_check(
                    f"{config['name']} Connection", True, "API key available"
                )
                return {
                    "success": True,
                    "message": f"{config['name']} is ready!",
                    "llm": config["name"],
                }
            else:
                pipeline_logger.log_system_check(
                    f"{config['name']} Connection", False, "No API key"
                )
                return {
                    "success": False,
                    "message": f"No API key for {config['name']}",
                    "llm": config["name"],
                }
        except Exception as e:
            pipeline_logger.log_system_check(
                f"{config['name']} Connection", False, str(e)
            )
