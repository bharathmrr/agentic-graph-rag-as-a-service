...
class RAGQueryRequest(BaseModel):
    ...
    include_entities: bool = Field(
        default=True, description="Whether to extract entities from response"
    )
...
class SimilarEntitiesRequest(BaseModel):
    entity_text: str = Field(
        ..., description="Entity text to find similar entities for"
    )
...
class GroqConfigRequest(BaseModel):
    api_key: Optional[str] = Field(
        None, description="Groq API key (optional if set in env)"
    )
    model: str = Field(default="llama3-8b-8192", description="Groq model to use")
    embedding_model: str = Field(
        default="BAAI/bge-small-en-v1.5", description="HuggingFace embedding model"
    )
...
async def get_groq_service() -> GroqRAGService:
    ...
    if groq_service is None:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to initialize Groq service: {str(e)}"
            )
    return groq_service
...
async def create_document_index(
    request: DocumentsRequest, service: GroqRAGService = Depends(get_groq_service)
):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to create document index"
            )
...
async def find_similar_entities(
    request: SimilarEntitiesRequest, service: GroqRAGService = Depends(get_groq_service)
):
    """Find entities similar to the given entity"""
    try:
        similar_entities = await service.find_similar_entities(
            request.entity_text, request.top_k
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error finding similar entities: {str(e)}"
        )


@router.post("/rag/query")
async def rag_query(
    request: RAGQueryRequest, service: GroqRAGService = Depends(get_groq_service)
):
    ...
    try:
        if request.stream:
            ...
            result = await service.query_with_rag(
                request.query, request.include_entities
            )
...
async def rag_query_stream(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error in streaming RAG query: {str(e)}"
        )
