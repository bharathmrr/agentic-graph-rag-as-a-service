...
async def get_job_status(job_id: str):
    ...
    try:
        ...
        return {"success": True, "job": status, "timestamp": datetime.now().isoformat()}
...
async def generate_embeddings(request: ProcessingRequest):
    ...
    try:
        ...
        text_chunks = [
            request.text[i : i + 500] for i in range(0, len(request.text), 500)
        ]
    ...
    async def generate_response():
        try:
            ...
            result = await llm_service.chat_response(
                request.query, request.context, progress_callback=progress_callback
            )
