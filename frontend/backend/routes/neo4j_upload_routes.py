...
async def upload_file_with_neo4j_integration(
    ...
    try:
        ...
        document_data = {
            ...
            "file_path": file_path,
            "content": (
                file_content.decode("utf-8", errors="ignore") if file_content else ""
            ),
        }
        ...
        if neo4j_pipeline and neo4j_pipeline.connected:
            background_tasks.add_task(
                process_document_pipeline, document_data, neo4j_pipeline
            )
        ...
        return UploadResponse(
            ...
            message=(
                ...
                else "File uploaded but Neo4j processing unavailable."
            ),
            document_id=document_id,
            filename=file.filename,
            size=file_size,
    ...
    try:
        ...
        if llm_service:
            ...
            if ontology_result.get("success"):
                ...
                logger.info(
                    f"Ontology generated successfully for document: {document_id}"
                )
            else:
                logger.error(
                    f"Ontology generation failed: {ontology_result.get('error')}"
                )
        ...
        if pipeline_result.get("success"):
            ...
            logger.error(
                f"Neo4j pipeline processing failed: {pipeline_result.get('error')}"
            )
        ...
        logger.error(
            f"Pipeline processing failed for document {document_data.get('document_id')}: {e}"
        )
    ...
    try:
        ...
        if verification_result.get("success"):
            return ProcessingStatus(
                ...
                status=(
                    "completed"
                    if verification_result.get("data_transferred")
                    else "processing"
                ),
                progress=100 if verification_result.get("data_transferred") else 50,
                current_step=(
                    "Data transferred to Neo4j"
                    if verification_result.get("data_transferred")
                    else "Processing..."
                ),
        ...
        return JSONResponse(
            ...
            content={
                ...
                "message": (
                    ...
                    if neo4j_pipeline.connected
                    else "Neo4j connection failed"
                ),
