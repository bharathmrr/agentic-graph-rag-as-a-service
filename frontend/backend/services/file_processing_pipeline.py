...
from .enhanced_llm_service import EnhancedLLMService, create_gemini_service, create_groq_service
...
class FileProcessingPipeline:
    ...
    async def notify_progress(self, job_id: str, stage_name: str, progress: int, message: str):
    ...
    async def _process_upload_stage(self, job: FileProcessingJob):
        ...
        try:
            await self.notify_progress(job.job_id, "upload", 10, "📁 Validating file...")
            ...
            await self.notify_progress(job.job_id, "upload", 50, "📄 Reading file content...")
            ...
            await self.notify_progress(job.job_id, "upload", 100, "✅ File upload validated!")
    ...
    async def _process_ontology_stage(self, job: FileProcessingJob):
        ...
        try:
            await self.notify_progress(job.job_id, "ontology", 10, "🧠 Initializing ontology generation...")
            ...
            result = await self.llm_service.generate_ontology(
                job.file_content,
                progress_callback=ontology_progress
            )
    ...
    async def _process_entity_stage(self, job: FileProcessingJob):
        ...
        try:
            await self.notify_progress(job.job_id, "entities", 10, "🔍 Starting entity extraction...")
    ...
    async def _process_embedding_stage(self, job: FileProcessingJob):
        ...
        try:
            await self.notify_progress(job.job_id, "embeddings", 10, "🔢 Preparing text for embeddings...")
    ...
    async def _process_graph_stage(self, job: FileProcessingJob):
        ...
        try:
            await self.notify_progress(job.job_id, "graph", 20, "🕸️ Building knowledge graph...")
            ...
            await self.notify_progress(job.job_id, "graph", 60, "🔗 Creating relationships...")
            ...
            await self.notify_progress(job.job_id, "graph", 100, "✅ Knowledge graph constructed!")
    ...
    async def _process_storage_stage(self, job: FileProcessingJob):
        ...
        try:
            await self.notify_progress(job.job_id, "storage", 20, "💾 Preparing data for storage...")
            await asyncio.sleep(0.5)
            
            await self.notify_progress(job.job_id, "storage", 50, "🗄️ Storing in vector database...")
            await asyncio.sleep(0.5)
            
            await self.notify_progress(job.job_id, "storage", 80, "📊 Updating graph database...")
            await asyncio.sleep(0.5)
            ...
            storage_summary = {
                ...
                "embeddings_stored": bool(job.results.get("embeddings", {}).get("success")),
                "graph_stored": bool(job.results.get("graph", {}).get("success")),
                "storage_timestamp": datetime.now().isoformat()
            }
            
            await self.notify_progress(job.job_id, "storage", 100, "✅ Data storage completed!")
